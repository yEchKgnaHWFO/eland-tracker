...
ElandTracker.storageAvailable=function(a){var d;try{if(d=window[a],!d)return!1}catch(k){return!1}try{return d.setItem("__storage_test__","__storage_test__"),d.removeItem("__storage_test__"),!0}catch(k){return!1}};
...
ElandTracker.getIframeUrl=function(a){function d(g){var b=new Date;b.setTime(b.getTime()+1E3);var a="expires\x3d"+b.toUTCString();g=g+"\x3d"+b.toDateString()+"; "+a+"; path\x3d/";document.cookie=g}function k(g){for(var b=document.cookie.split(";"),a=0;a<b.length;a++){var f=b[a];if(1<f.length)for(;" "===f.charAt(0);)f=f.substring(1);if(0===f.indexOf(g))return f.substring(g.length,f.length)}return""}function h(){for(var a=location.search.substring(1).split("\x26"),b=0;b<a.length;b++){var c=a[b].split("\x3d");
if("utm_medium"===c[0])try{return decodeURIComponent(c[1])}catch(f){return c[1]}}return""}function e(){for(var a=location.search.substring(1).split("\x26"),b=0;b<a.length;b++){var c=a[b].split("\x3d");if("utm_campaign"===c[0])try{return decodeURIComponent(c[1])}catch(f){return c[1]}}return""}function l(){for(var a=location.search.substring(1).split("\x26"),b=0;b<a.length;b++){var c=a[b].split("\x3d");if("utm_term"===c[0])try{return decodeURIComponent(c[1])}catch(f){return c[1]}}return""}function f(){for(var a=
location.search.substring(1).split("\x26"),b=0;b<a.length;b++){var c=a[b].split("\x3d");if("utm_content"===c[0])try{return decodeURIComponent(c[1])}catch(f){return c[1]}}return""}function q(){for(var a=location.search.substring(1).split("\x26"),b=0;b<a.length;b++){var c=a[b].split("\x3d");if("utm_source"===c[0])try{return decodeURIComponent(c[1])}catch(f){return c[1]}}return ElandTracker.getCampaignCodeFromStorage("elandUtmSource")}function r(){for(var a=location.search.substring(1).split("\x26"),
...
b+="\x26gid\x3d"+this.googleId)}ElandTracker.getFingerPrintV4&&(b="Android"===this.os||"iOS"===this.os?b+"\x26fp\x3d":b+"\x26fp2\x3d",b+=ElandTracker.getFingerPrintV4+"\x26");a.p1&&(b+="\x26p1\x3d"+a.p1);a.p2&&(b+="\x26p2\x3d"+a.p2);a.p3&&(b+="\x26p3\x3d"+a.p3);sessionStorage.removeItem("nopd");sessionStorage.removeItem("usefp");return b}(a)};
...
ElandTracker.getClickForceIframeUrl=function(a){function d(a,d){var e=a.split("/"),g="";if(4<=e.length)for(var b=0;b<d;b++)if(4===e.length&&0===e[3].length){g="Web-Index";break}else if(e.length>3+b)g+=e[3+b]+"-";else break;else g="Web-Index";return g=g.split("?")[0]}function k(a){return a.toLowerCase().replace("\x3e","").replace("\x3c","").replace(";","").replace("script","").replace("alert","").replace("img","").replace("frame","").replace("input","").replace("title","").replace("body","")}var h=
...
h.BlackBerry()||h.iOS()||h.Opera()||h.Windows()||h.Puffin()||h.Other()}};this.subfolderDepth=a.trackSubfolderDepth?a.trackSubfolderDepth:1;this.target=a.targetType?a.targetType:"ad";this.url=a.url?a.url:k(location.href);this.referUrl=a.referUrl?a.referUrl:k(document.referrer);this.adSrTag=function(){for(var a=location.search.substring(1).split("\x26"),d=0;d<a.length;d++){var e=a[d].split("\x3d");if("AD_SR"==e[0]||"ad_sr"==e[0])return e[1]}return""}();a.cf_uid&&(this.cf_uid=a.cf_uid);for(var e=a.sp_variable.split(","),
l=0;l<e.length&&4>l;l++)null!=this.info?(this.info+=",",this.info+=e[l]):this.info=e[l];this.dmpSr=a.source?a.source:location.hostname;if(50<this.dmpSr.length&&0>=this.dmpSr.length)throw e="[ElandTracker] The length of 'DMP_SR' is invalid.",console.log(e),Error(e);this.trackType=a.trackType?a.trackType:"view";this.os=function(a){return a.match(/Windows Phone/i)?"WindowsPhone":a.match(/Android/i)?"Android":a.match(/iPhone|iPad|iPod/i)?"iOS":a.match(/Windows NT/i)?"Windows":a.match(/Macintosh/i)?"MacOS":
